...
import { useDisbursements, useCreateDisbursement, useApproveDisbursement, useMarkDisbursementPaid } from '@/features/finance/hooks/use-finance'
import type { DisbursementFilters, DisbursementVO, CreateDisbursementDTO } from '@/features/finance/types/finance.types'
...
export default function DisbursementsPage() {
  ...
  const [filters, setFilters] = useState<DisbursementFilters>({ page: 1, perPage: 20 })
  const [search, setSearch] = useState('')
  const [statusFilter, setStatusFilter] = useState('')
  const [showForm, setShowForm] = useState(false)

  const { data, isLoading } = useDisbursements({ ...filters, search: search || undefined, status: statusFilter || undefined })
  const approveMut = useApproveDisbursement()
  const markPaidMut = useMarkDisbursementPaid()
    ...
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && setFilters((f) => ({ ...f, page: 1 }))} placeholder={`${tc('search')} ${t('disb_no')}...`} className="w-full rounded-md border bg-[var(--background)] pl-9 pr-4 py-2 text-sm outline-none focus:ring-2 focus:ring-[var(--ring)]" />
        </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-[var(--muted-foreground)]">
          <CreditCard className="h-12 w-12 mb-3" /><p>{tc('no_data')}</p>
        </div>
      ) : (
        <div className="overflow-x-auto rounded-lg border">
          <table className="w-full text-sm">
            <thead className="bg-[var(--accent)] text-left">
              <tr>
                <th className="px-4 py-3 font-medium">{t('disb_no')}</th>
            ...
            <tbody className="divide-y">
              {items.map((row: DisbursementVO) => (
                <tr key={row.id} className="hover:bg-[var(--accent)]/50">
                  <td className="px-4 py-3 font-mono text-xs">{row.disbNo}</td>
      ...
      {/* 新建付款 dialog */}
      {showForm && <DisbursementFormDialog onClose={() => setShowForm(false)} />}
    </div>
  )
}

/** 新建付款表单 */
function DisbursementFormDialog({ onClose }: { onClose: () => void }) {
  const t = useTranslations('finance')
  const tc = useTranslations('common')
  const createMut = useCreateDisbursement()

  const [form, setForm] = useState<CreateDisbursementDTO>({
    supplierId: '', amount: 0, currency: 'TWD', method: 'transfer',
  })
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
      <div className="w-full max-w-md rounded-lg bg-[var(--card)] p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold">{tc('create')} {t('disbursements')}</h2>
          <button onClick={onClose}><X className="h-5 w-5" /></button>
        </div>
        <div className="flex flex-col gap-3">
          <label className="text-sm font-medium">{t('supplier')}
            <input type="text" value={form.supplierId} onChange={(e) => setForm({ ...form, supplierId: e.target.value })} className="mt-1 w-full rounded-md border bg-[var(--background)] px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-[var(--ring)]" />
          </label>
          ...
          <label className="text-sm font-medium">{t('purchase_ref')}
            <input type="text" value={form.purchaseId ?? ''} onChange={(e) => setForm({ ...form, purchaseId: e.target.value })} className="mt-1 w-full rounded-md border bg-[var(--background)] px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-[var(--ring)]" />
          </label>
